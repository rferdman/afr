...
int PhaseShift(struct Polyco *Polycos, int n_poly, struct StdProfs *Profile, 
  ...
  // printf("%.5lf : oldphase=%.7lf  newphase=%.7lf\noldperiod=%lf  newperiod=%lf\n", Hdr->obs.ChanFreq[chan], SubHdr->DumpRefPhase[chan], RefPhase, SubHdr->DumpRefPeriod[chan], 1./RefFreq);
